...
import { addAmmunition, drawEquipment, getEquippedItems, removeItemById, getWeapon } from './libs/weaponMacrosCTA';
import { getMeleeModifiers, getRangedModifiers } from './actions/modifiers';
import { getHitLocationsObject, getLocationData } from './libs/locationsDataTransformation';
...
export default class AttackChooser extends BaseActorController {
  constructor(token, data = {}, promiseFuncs, maneuver) {
  ...
  async getData() {
    ...
    const weaponsInHands = await getWeaponsInHands(this.token.document);
    ...
    this.locationToAttack = this.token.document.getFlag(MODULE_NAME, 'location');
  ...
  async calculateAttack(mode, index, element) {
    const isUsingFatigueForMoveAndAttack = $('#fatigueMoveAndAttack').is(':checked');
  ...
  async showModifiers(mode, index, element) {
    setTimeout(async () => {
  ...
  activateListeners(html) {
    ...
    activateChooser(
      html,
      'disarm_attacks',
      (index) => this.makeAttack('disarm_attack', index, undefined),
      (index, element) => this.showModifiers('disarm_attack', index, element),
    );
    activateChooser(
      html,
      'counter_attacks',
      (index) => this.makeAttack('counter_attack', index, undefined),
      (index, element) => this.showModifiers('counter_attack', index, element),
    );
    activateChooser(
      html,
      'melee_attacks',
      (index) => this.makeAttack('melee', index, undefined),
      (index, element) => this.showModifiers('melee', index, element),
    );
    activateChooser(
      html,
      'ranged_attacks',
      (index, element) => this.makeAttack('ranged', index, element),
      (index, element) => this.showModifiers('ranged', index, element),
    );
  ...
  async unReadyWeapon(weapon) {
    ...
    if (this.actor) {
      ...
      const rangedAttack = ranged.find((i) => i.itemid === weapon.itemid);
      if (rangedAttack) {
    ...
    await removeItemById(this.actor.tokenSelected.id, weapon.itemid);
  ...
  async unReadyWeaponChooser(index) {
    const weapon = this.weaponsNotToBeReadyData[index];
    return this.unReadyWeapon(weapon);
  }
  async fastDrawSkillCheck(weapon, remainingRounds) {
    const readyActionsWeaponNeeded = this.actor.getReadyActionsWeaponNeeded();
    for (const SKILL of Object.keys(FAST_DRAW_SKILLS)) {
      ...
      if (fastDrawSkill) {
        ...
        const hasWeapon = SKILLS.find((element) => {
  ...
  async makeAttack(mode, index, element) {
    ...
    const weapon = this.actor.getWeaponFromAttack(attack);
    ...
    if (mode === 'melee') {
      ...
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      const distance = game.canvas.grid.measureDistance(attacker, defender, { gridSpaces: true }) || 0;
      const reach = attack.reach.replace('C', '0').split(',');
  ...
  async chooseLocation(index, element) {
    ensureDefined(game.user, 'game not initialized');
    const target = getTargets(game.user)[0];
    ensureDefined(target.actor, 'target has no actor');
...
AttackChooser.modifiersGetters = {
  melee: getMeleeModifiers,
  ranged: getRangedModifiers,
};
//# sourceMappingURL=attackChooser.js.map
