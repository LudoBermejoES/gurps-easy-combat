...
export default class DefenseChooser extends BaseActorController {
  ...
  async showModifiers(mode, index, element) {
    setTimeout(async () => {
      ...
      if (mode === 'PARRY') {
        const parries = await this.actor.getValidParries(this.token, this.data, this.canUseModShield);
  ...
  async checkResult(result, mode) {
    if (!result) {
      if (getManeuver(this.actor) === 'aod_double') {
        const lastAodDouble = this.token.document.getFlag(MODULE_NAME, 'lastAodDouble');
  ...
  activateListeners(html) {
    ...
    activateChooser(
      ...
      (index, element) => this.showModifiers('PARRY', index, element),
    );
    activateChooser(
      html,
      'blocks',
      async (index) => {
        ...
        const blocks = await this.actor.getValidBlocks(this.token, this.data, this.canUseModShield);
      ...
      (index, element) => this.showModifiers('BLOCK', index, element),
    );
  }
  static async attemptDefense(sceneId, tokenId, attackerId, modifiers, canUseModShield) {
  ...
  static async requestDefense(token, modifiers, attackerTokenId, canUseModShield) {
    const actor = token.actor;
...
//# sourceMappingURL=defenseChooser.js.map
